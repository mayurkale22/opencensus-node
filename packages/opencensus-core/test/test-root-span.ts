...
describe('RootSpan', () => {
  ...
  describe('new traceId()', () => {
    it('should get trace id from rootspan instance', () => {
      ...
      assert.strictEqual(
        root.oTelSpan.context().traceId,
        child.oTelSpan.context().traceId
      );
    });
  });
  ...
  describe('addAnnotation()', () => {
    it('should add an annotation', () => {
      ...
      const oTelSpanEvents = (rootSpan.oTelSpan as oTelTracing.Span).events;
      assert.ok(oTelSpanEvents.length > 0);
      assert.strictEqual(oTelSpanEvents[0].name, 'description test');
      assert.deepStrictEqual(oTelSpanEvents[0].attributes, {});
    });
