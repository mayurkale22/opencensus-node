...
export class StackdriverStatsExporterUtils {
  ...
  static createPoint(
    ...
    if (valueType === ValueType.INT64) {
      ...
      // this.logger.warn(`${valueType} is not supported.`);
    }

    // TODO(mayurkale): Decide how to convert current process.hrtime to Date
    // format or get ride of process.hrtime for startTimeStamp.
    // const endTime = (new Date(new Date().getTime() + (1000 *
    // 30))).toISOString(); const startTime = (new Date()).toISOString();

    const endTime = '2019-01-04T00:25:49.522Z';
    const startTime = '2019-01-04T00:15:49.522Z';
