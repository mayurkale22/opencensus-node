...
import {StackdriverStatsExporterUtils} from './stackdriver-stats-utils';
...
export class StackdriverStatsExporter implements StatsEventListener {
  ...
  async export() {
    ...
    // metricProducerManager.getAllMetricProducer().forEach(metricProducer => {
    //   metricProducer.getMetrics().forEach(async metric => {
    //     console.log('in s');
  ...
  private createMetricDescriptor(metricDescriptor: OCMetricDescriptor):
    ...
    return this.authorize().then((authClient) => {
      const request = {
        name: `projects/${this.projectId}`,
        resource: StackdriverStatsExporterUtils.createMetricDescriptorData(
